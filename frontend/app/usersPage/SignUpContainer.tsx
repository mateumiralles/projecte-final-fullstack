...
export default function SignUpContainer(props: SignUpContainerProps) {
  ...
  const checkErrors1 = () => {
    if (name == "") {
      setErrorMsg("You must fill the name input");
      setRegisterError(true);
      return true;
    } else if (name.length <= 1) {
      setErrorMsg("Your name is too short");
      setRegisterError(true);
      return true;
    } else if (surname == "") {
      setErrorMsg("You must fill the surname input");
      setRegisterError(true);
      return true;
    } else if (email == "") {
  ...
  const OnRegister = async () => {
    if (!checkErrors2()) {
      setRegisterError(false);
      try {
        const respuesta = await axios.post("http://localhost:3333/api/users/", {
          name: name,
          lastName: surname,
          email: email,
          password: pass,
