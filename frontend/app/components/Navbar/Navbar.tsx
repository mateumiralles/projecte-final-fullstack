...
export default function Navbar() {
  ...
  const Logout = async (id: number) => {
    try{
        const logout = await axios.post(`http://localhost:3333/api/users/${id}/logout`);
  ...
  const getUser = async () => {
    ...
    if(userLocal){
      setUser(userLocal);
      try{
        const obtainUser = await axios.get(`http://localhost:3333/api/users/${userLocal.id}`);
        if(obtainUser.status===200){
          if(obtainUser.data.token===null){
            ...
            try{
              const userCart = await axios.get(`http://localhost:3333/api/users/${obtainUser.data.id}/cart`);
      ...
      <div className="flex-4 mr-14 flex flex-row gap-5 ">
        {user ? (
          <button onClick={() => setProfile(!profile)}>
            ...
            {profile ? <UserProfile setProfile={setProfile} logout={Logout} user={user}/> : null}
          </button>
        ) : (
          <Link href="/usersPage">
            <NavbarIconButton svgName="/userIcon.svg" />
